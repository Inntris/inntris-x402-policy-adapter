...
const FAILURE_ORDER = new Map<ConformanceFailureCode, number>(
  CONFORMANCE_FAILURE_CODES.map((code, index) => [code, index]),
);

export class FailureCollector {
  readonly #codes = new Set<ConformanceFailureCode>();

  add(code: ConformanceFailureCode, condition = true): void {
    if (condition) this.#codes.add(code);
  }

  has(code: ConformanceFailureCode): boolean {
    return this.#codes.has(code);
  }

  ordered(): ConformanceFailureCode[] {
    return [...this.#codes].sort(
      (left, right) => (FAILURE_ORDER.get(left) ?? 0) - (FAILURE_ORDER.get(right) ?? 0),
    );
  }
}
